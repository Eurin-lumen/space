...
function registerAdmin($username, $password, $confirm_password) {
    ...
    if (mysqli_stmt_num_rows($stmt) > 0) {
        return false; // L'administrateur existe déjà
    }
    ...
    if (mysqli_stmt_execute($stmt)) {
        return true; // L'administrateur a été enregistré avec succès
    } else {
...
function addBook($title, $author, $domain, $description, $copies, $image, $pdf) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        // Obtenir le nom temporaire du fichier téléchargé
        $tmp_name = $_FILES['image']['tmp_name'];

        // Définir le chemin où l'image sera stockée sur le serveur
        $image_filename = 'images/' . $_FILES['image']['name'];

        // Déplacer le fichier téléchargé vers son emplacement final
        move_uploaded_file($tmp_name, $image_filename);
    } else {
        // Si l'image n'a pas été téléchargée, définir le nom de fichier à null
        $image_filename = null;
    }

    // Préparer la requête SQL pour insérer le nouveau livre
    $sql = "INSERT INTO books (title, author, domain, description, copies, image_filename, pdf) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $sql);
    ...
    mysqli_stmt_bind_param($stmt, "ssssiss", $title, $author, $domain, $description, $copies, $image_filename, $pdf);
...
///METTRE A JOUR A PROBLEME
// Fonction pour mettre à jour les informations d'un livre
function updateBook($book_id, $title, $author, $domain, $description, $copies, $image, $pdf) {
    global $conn;

    // Préparer la requête SQL pour mettre à jour les informations du livre
    $sql = "UPDATE books SET title = ?, author = ?, domain = ?, description = ?, copies = ?, image = ?, pdf = ? WHERE id = ?";
    $stmt = mysqli_prepare($conn, $sql);
    ...
    mysqli_stmt_bind_param($stmt, "ssssisssi", $title, $author, $domain, $description, $copies, $image, $pdf, $book_id);

    // Exécuter la requête
    if (mysqli_stmt_execute($stmt)) {
        return true; // Le livre a été mis à jour avec succès
    } else {
        return false; // Une erreur s'est produite lors de la mise à jour du livre
    }
}
...
function deleteBook($book_id) {
    ...
    // Exécuter la requête
    if (mysqli_stmt_execute($stmt)) {
        return true; // Le livre a été supprimé avec succès
    } else {
